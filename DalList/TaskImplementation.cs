...
internal class TaskImplementation : ITask
{
    public int Create(Task item)
    {
        ...
        Task newTask = new Task(newId, item.Description, item.Alias, item.IsMileStone,item.RequiredEffortTime, item.CreatedAtDate, item.StartedDate, item.ScheduledDate, item.ForeCastDate, item.DeadLineDate, item.CompleteDate, item.Deliverables, item.Remarks, item.EngineerId, item.ComplexityLevel);
